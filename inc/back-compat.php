 ...
 *
 * Prevents Tortuga from running on WordPress versions prior to 4.4,
 ...
 * Tortuga on WordPress versions prior to 4.4.
 *
 * @global string $wp_version WordPress version.
 */
function tortuga_upgrade_notice() {
	$message = sprintf( esc_html__( '%1$s requires at least WordPress version %2$s. You are running version %3$s. Please upgrade and try again.', 'tortuga' ), 'Tortuga', '4.4', $GLOBALS['wp_version'] );
...
function tortuga_customize() {
	wp_die( sprintf( esc_html__( '%1$s requires at least WordPress version %2$s. You are running version %3$s. Please upgrade and try again.', 'tortuga' ), 'Tortuga', '4.4', $GLOBALS['wp_version'] ), '', array(
		'back_link' => true,
	) );
}
...
function tortuga_preview() {
	if ( isset( $_GET['preview'] ) ) {
		wp_die( sprintf( esc_html__( '%1$s requires at least WordPress version %2$s. You are running version %3$s. Please upgrade and try again.', 'tortuga' ), 'Tortuga', '4.4', $GLOBALS['wp_version'] ) );
	}
}
