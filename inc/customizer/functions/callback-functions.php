...
function tortuga_slider_activated_callback( $control ) {
	...
	if ( $control->manager->get_setting('tortuga_theme_options[slider_blog]')->value() == 1 ) :
		return true;
	elseif ( $control->manager->get_setting('tortuga_theme_options[slider_magazine]')->value() == 1 ) :
		return true;
	else :
		return false;
	endif;
